...
python3 banned_words_checker.py --solution=smart_pointers.hpp --banned-words=banned_words.json
if [[ ! $? -eq 0 ]]
then
...
clang-tidy -extra-arg=-std=c++20 -quiet smart_pointers.hpp
if [[ ! $? -eq 0 ]]
then
...
clang-format -i -style='{BasedOnStyle: Google, DerivePointerAlignment: false, PointerAlignment: Left, AlignOperands: true}' smart_pointers.hpp &&
git diff --ignore-submodules --color smart_pointers.hpp > diff &&
cat diff
if [[ -s diff ]]
then
